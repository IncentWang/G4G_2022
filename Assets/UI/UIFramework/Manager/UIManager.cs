...
namespace UIFramework
{
    public class UIManager
    {
        ...
        readonly string jsonPath = Application.dataPath + @"/UI/Json/UIJson.json";
        ...
        public void InitializePanelDict()
        {
            panelDict = new Dictionary<UIPanelType, BasePanel>();
        }

        /// <summary>
        /// 检测当前活动的窗口与输入的UIPanelType是否匹配
        /// </summary>
        /// <param name="uIPanelType"></param>
        /// <returns></returns>
        public bool ActivityUIPanelType(UIPanelType uIPanelType)
        {
            if (panelStack.Count > 0)
            {
                BasePanel topPanel = panelStack.Peek();
                if (panelDict.ContainsValue(topPanel))
                {
                    foreach (var dic in panelDict)
                    {
                        if (dic.Value == topPanel)
                        {
                            return dic.Key == uIPanelType;
                        }
                    }
                }
